...
public class EndToEndTest {
	...
	Response response;
	String BaseURI = "http://3.89.194.229/employees/";
	...
	public void test9() {
		...
		response = PostMethod("David", "Jackson", "5000", "david@email.com");
		...
		response = PutMethod(EmpId, "Bleck", "Kernel", "4000", "bleck@email.com");
		Assert.assertEquals(response.getStatusCode(), 200);
		System.out.println("Status Code: " + response.getStatusCode());
		
		jpath = response.jsonPath();			
		String EmpName = jpath.get("firstName");
		Assert.assertEquals(EmpName, "Bleck");
